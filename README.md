...
Use `npm start` to open on a physical device with a compatible Expo Go app, or `npm run web` for a browser preview. A local iOS simulator requires Xcode.
...
Choices are saved on Continue and completion using SQLite key-value storage on native devices and localStorage in the web preview. Completion stays on screen three with a saved state; reopening restores that state. The tracker home screen is intentionally outside this build.

Finishing with a reminder frequency requests notification permission and schedules one recurring local check-in. Denied permission still saves onboarding, with a clear status. Choosing no notifications cancels the onboarding check-in. The browser preview saves the preference but does not schedule notifications. These are general check-ins, not per-tracker due-date notifications.
