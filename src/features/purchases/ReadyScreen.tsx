...
export default function ReadyScreen({ setup }: { setup: Setup }) {
  ...
  useEffect(() => {
    if (simulated) {
      setMessage('Simulated purchase complete. No charge was made and no notifications were scheduled.');
      return;
    }
    let active = true;
    setFailed(false);
    ...
    return () => { active = false; };
  }, [setup, attempt, simulated]);
  return <SafeAreaView style={s.safe}><View style={s.content}><View style={s.icon}><Icon name="check" size={34} color={c.blueDark} /></View><Text style={s.title}>A little more{ '\n' }peace of mind.</Text><Text style={s.body}>{simulated ? 'Dueish Pro preview is active.' : 'Dueish Pro is active. Your list is ready.'}</Text><Text style={s.note}>{message}</Text>{simulated && <Pressable accessibilityRole="button" onPress={resetSimulation} style={{ paddingVertical: 18 }}><Text style={s.body}>Test another plan</Text></Pressable>}{failed && <Pressable accessibilityRole="button" onPress={() => setAttempt(value => value + 1)} style={{ padding: 18 }}><Text style={s.body}>Retry reminders</Text></Pressable>}</View></SafeAreaView>;
}
